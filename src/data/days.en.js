...
const englishSlides = {
  ...
  2: [
    ...
    },
    {
      ...
      body: 'Supabase → New project. Then open Project Settings → API Keys and copy Project URL and the Publishable key into .env.',
    ...
    {
      kicker: 'Step 4 · connect the database', title: 'Two commands in the terminal',
      body: 'Type them yourself, one at a time. REF comes from your database address: https://REF.supabase.co. The database password is typed directly into the terminal — the characters stay invisible, that is normal.',
      visual: { code: 'npm run db:login                       ← sign in (approve in browser)\nnpm run db:link -- --project-ref REF   ← link your project' },
      note: 'Paste only commands from code blocks into the terminal — never plain text.',
    },
    {
      kicker: 'Step 5 · tables', title: 'Apply the migrations',
      body: 'Migrations are files that describe your tables. First preview what will be applied, then apply it.',
      visual: { code: 'npm run db:push -- --dry-run   ← preview what will be applied\nnpm run db:push -- --yes       ← create the tables (entries)' },
      note: 'If you see “Remote database is up to date”, everything is already done.',
    },
    {
      kicker: 'Step 6 · keep-alive', title: 'Keep the database awake',
      body: 'One command: it checks the database, stores settings in GitHub Secrets, and starts a daily ping robot. Success = the line “Keep-alive verified” with a link.',
      visual: { code: 'npm run keep-awake:setup' },
      note: 'A gh error? Once: gh auth login → GitHub.com → HTTPS → browser. Then run setup again.',
    },
    {
      kicker: 'Step 7 · test', title: 'Run it locally first',
    ...
    { kicker: 'Test', title: 'Open the URL on your phone', body: 'Register and add an entry from your phone. Does it work? Your product is live online 🎉' },
    {
      ...
      visual: { head: ['What you see', 'What to check'], rows: [
        ['White screen / key error', '.env and both variables'],
